...
class ATL_NO_VTABLE CRTCStatsReport :
	...
	public IDispatchImpl<IRTCStatsReport, &IID_IRTCStatsReport, &LIBID_webrtceverywhereLib, /*wMajor =*/ 1, /*wMinor =*/ 0>
{
public:
	CRTCStatsReport();
...
public:
	WE_INLINE void SetStats(std::shared_ptr<_RTCStatsReport> & states) { m_Stats = states; }
	WE_INLINE std::shared_ptr<_RTCStatsReport> GetStats() { return m_Stats; }

	STDMETHOD(result)(__out VARIANT* RTCStatsList);

private:
	std::shared_ptr<_RTCStatsReport> m_Stats;
